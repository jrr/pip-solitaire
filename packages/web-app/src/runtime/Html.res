...
type elementProps = {
  ...
  // --- links out. The app renders two `<a>`s — the game info screen's Wikipedia
  // article and the About screen's repository — and all three of these belong to them.
  // `target` is optional because whether a link out asks for a tab of its own is a
  // platform question, answered in one place (`LinkOut`); `rel` because a page opened
  // in one can otherwise reach back through `window.opener`.
  href?: string,
  target?: string,
  rel?: string,
