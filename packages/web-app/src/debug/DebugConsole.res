...
//     spliced into the shell with `Html.node`. Deliberately outside the diff: the
//     cost to avoid is the walk, not the patching. Rendering the log as JSX would
//     hand the diff up to 500 vnodes on every entry, and entries arrive in bursts
//     (an autoplay narrates every move). Appending one `<li>` and dropping one from
//     the front is work proportional to what changed instead.
//
