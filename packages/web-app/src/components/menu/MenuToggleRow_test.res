...
describe("MenuToggleRow", () => {
  test("shows the label and the line explaining what the setting does", () => {
    // Both halves: a label with no description leaves a setting unexplained.
  ...
  test("announces its state as well as showing it", () => {
    ...
    // screen reader can read — a `<button role="switch">` has no checkedness of its
    // own — so both have to move together. What each class
