...
describe("GameVariant.forVariant", () => {
  test("draws a pack as the suits in play, in the deck's own order", () => {
    expect(markFor(Game.spiderette).mark)->toEqual(
      GameVariant.Pips({suits: "♠♥", copies: Some("×2")}),
    )
    expect(markFor(Game.spiderette1).mark)->toEqual(
      GameVariant.Pips({suits: "♠", copies: Some("×4")}),
    )
    // The standard pack, once over: "♠♥♦♣ ×1" would be the deck with a number after it
    // that a reader has to discard.
    expect(markFor(Game.spiderette4).mark)->toEqual(
      GameVariant.Pips({suits: "♠♥♦♣", copies: None}),
    )
  })
  ...
  test("reads a pack off any deck, not off a table of the packs there happen to be", () => {
    ...
    expect(GameVariant.pips(Game.micro.deck).mark)->toEqual(
      GameVariant.Pips({suits: "♠♥", copies: None}),
    )
  })
})
