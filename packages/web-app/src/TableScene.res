...
let make = (
  ...
  // Read once, here, as the scene is *built* — the bug. See the mount below.
  let seededAtBuildTime = loadHistory()
  {
    id: game.id,
    label: game.name,
    mount: container => {
      ...
      let startShake = () =>
        ...
        | Some(_) => ()
        | None =>
          shakeUnsub := Some(Motion.subscribeShake(~onShake=() => boardOps.contents.jostle()))
        }
      let stopShake = () => {
        unsubscribeShake()
        ...
        let makeCard = (cardData: Deck.card) => {
          ...
          let endDrag = ev =>
            ...
            | Some((_, _, spanStarts)) =>
              ...
              grab := None
              spanStarts->Array.forEach(((c, _, _)) =>
                classList(c.wrapper)->removeClass("dragging")
              )
      ...
      // deal. The save is read from `seededAtBuildTime` — once, when the scene was
      // built — which is the bug the accompanying test pins: every mount after the
      // first re-opens that snapshot over the game actually being played.
      buildBoard(~initial?, ~history=?seededAtBuildTime, game)
