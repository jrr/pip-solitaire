...
test.describe("in the menu", () => {
  ...
  test("leaves two quick taps on different controls alone", async ({ page }) => {
    ...
    // alone would eat the second control: open the menu and go straight for New Deal —
    // well inside 350ms — and New Deal would never fire. Distinct controls are further
    ...
    // `touchscreen.tap` on New Deal doesn't close the menu even with this module gone),
    ...
    const newGame = await page.getByRole("button", { name: "New Deal" }).boundingBox()
