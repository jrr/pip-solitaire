...
test("plays deal 7 from the opening layout to the win overlay", async ({ page }) => {
  ...
  // The app never once surprised `core`. After every single drag the harness
  // re-reads the board and compares it against the board `core` said the move would
  // leave behind (`Solver.planSteps`, planned over `Position`); `replans` counts the
  // times they differed. Zero means the running app — its pointer loop, its supermove
  // limit, its auto-collect, the point where `canFinish` suppresses it — behaved for a
  // whole game exactly as the pure rules say it should. A non-zero count is the
  // interesting failure: the *app* is doing something its own core doesn't predict.
  // (The rules themselves are checked against the reducer in `core`'s `Position_test`,
