...
// crash.
//
// **The format, the version policy, and the pipeline this feeds are documented in
// `docs/save-and-share.md`.** Read that before changing the shape.
//
// It lives in `core`, beside the state it serializes, for two reasons: the encoding is
// pure (no `localStorage`, no `Math.random`) and so is unit-testable here, and keeping
// the wire format next to `GameState`/`History` means a change to those types is a
// change to this file, not a silently-drifting copy in the view. The web app's
// `SavedGame` is the thin impure wrapper that only reads and writes browser storage.
open Card

// The wire-format version, and it **deliberately stays at 1**. Every field added since
// (`stats`, `autoplays`, `timing`, `game`) is optional in both directions, which is what
// a bump would have cost: every saved game on every device dropped, and every share link
// already in a chat turned into "couldn't read that". Reserve the bump for a change that
// genuinely can't be read both ways — see `docs/save-and-share.md` before spending it.
//
// `decode` rejects any other version, so an old save is dropped and the player gets a
// fresh deal instead of a misread board.
let version = 1
...
type t = {
  ...
  // isn't in this build's list is exactly the case a reader has to be able to reject.
...
// the one thing such a save can still say about how it got there. `undos` falls back to
// none: an undo leaves no trace in a history, so a save that didn't count them has no
// way to remember, and guessing high would be a worse lie than guessing zero.
...
// A save for a history with nothing recorded beside it: what any caller holding only a
// history starts from. The tally is inferred above; the clock and the game aren't
// inferred at all, because a bare history is a line of play with no deal time and no
// board attached, and guessing at either would be inventing the one fact this save was
// built without.
...
// `GameState.t` and a `Game.t` have to agree on for the board to render at all.
...
// (midnight, 1970) that `decode` would believe.
