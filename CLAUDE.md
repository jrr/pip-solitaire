...
The bindings live in `packages/web-app/src/runtime/Html.res`; `docs/rendering.md`
has the pipeline and the two things that bite.

The short version: the same arrangement has to be stated in **three** places, in
three different vocabularies (`res-jsx-plugin.js`, `vite.config.js`'s
`optimizeDeps`, and `scripts/lib/load-jsx-module.mjs`) with nothing checking that
they agree — `mise run dev-smoke` is what catches it — and bare Node can't
`import` compiled output that carries JSX, so a build script goes through
`load-jsx-module.mjs`.
