...
`web-app/src/scenes/TableScene.css` (`.stacking-card .card-art`). The opening-board
preview on a game's info screen (`web-app/src/cards/BoardArt.res`) applies the same
hash as an SVG `rotate`, so a card it shows tilted is a card the player will find
at that angle.
...
The info screen's preview reads the same setting, as a prop from the settings
